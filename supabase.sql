...
-- =============================================
-- Auto-swap roster when a shift exchange is approved
-- =============================================
CREATE OR REPLACE FUNCTION apply_shift_exchange()
RETURNS trigger AS $$
BEGIN
  IF NEW.status = 'approved' THEN
    -- requester takes the target shift
    UPDATE shifts SET user_id = NEW.target_user_id WHERE id = NEW.original_shift_id;
    -- target user takes the requester shift
    UPDATE shifts SET user_id = NEW.requester_id WHERE id = NEW.target_shift_id;
  END IF;
  RETURN NEW;
END;
$$ LANGUAGE plpgsql;

DROP TRIGGER IF EXISTS trg_apply_shift_exchange ON shift_exchanges;
CREATE TRIGGER trg_apply_shift_exchange
AFTER UPDATE ON shift_exchanges
FOR EACH ROW
WHEN (NEW.status = 'approved')
EXECUTE FUNCTION apply_shift_exchange();